...
export function VideoUploader({
  ...
  const [mode, setMode] = useState<Mode>(videoUrl && !videoUrl.startsWith('blob') ? 'link' : 'upload')
  const [linkInput, setLinkInput] = useState(videoUrl ?? '')
  ...
  function handleFile(file: File) {
    ...
    setError('')
    setProgress(0)
    const storageRef = ref(storage, `videos/${lessonId}/${file.name}`)
    ...
    <div className="mb-4">
        ...
        <div>
          {progress !== null ? (
            ...
            >
              ...
              <p className="text-[13px] text-ink-muted">Dra og slipp video her, eller <span className="text-accent font-medium">velg fil</span></p>
              <p className="text-[11px] text-ink-light mt-1">MP4, MOV, WebM</p>
          ...
          {videoUrl && progress === null && (
            <div className="flex items-center justify-between mt-2 px-1">
              <span className="text-[12px] text-green font-medium">Video lastet opp</span>
      ...
      {videoUrl && (() => {
        ...
        return embed.type === 'iframe' ? (
          <div className="relative w-full rounded-xl overflow-hidden mt-3 bg-black" style={{ paddingTop: '56.25%' }}>
            <iframe
              src={embed.src}
          ...
          <video src={embed.src} controls className="w-full rounded-xl mt-3 bg-black" style={{ maxHeight: 280 }} />
        )
      })()}
    </div>
  )
